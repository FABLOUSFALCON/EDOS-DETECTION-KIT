...
class PacketTime:
    ...
    def _get_packet_times(self):
        ...
        Returns:
            A list of the packet times in microseconds.

        """
        if self.packet_times is not None:
            return self.packet_times
        first_packet_time = self.flow.packets[0][0].time
        # Ensure times are in microseconds
        if first_packet_time < 1e10:
            first_packet_time *= 1000000
        packet_times = []
        for packet, _ in self.flow.packets:
            pkt_time = packet.time
            if pkt_time < 1e10:
                pkt_time *= 1000000
            packet_times.append(float(pkt_time - first_packet_time))
        return packet_times
