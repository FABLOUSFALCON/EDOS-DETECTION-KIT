...
class PacketLength:
    ...
    def _get_payload_length(self, packet):
        """Get payload length matching Java CICFlowMeter getPayloadBytes()"""
        if "IP" not in packet:
            return 0
        ...
        # IP header size
        ip_header_size = packet["IP"].ihl * 4  # IHL is in 4-byte words
        ...
        if "TCP" in packet:
            ...
            )  # TCP header length in 4-byte words
        elif "UDP" in packet:
            transport_header_size = 8  # UDP header is always 8 bytes

        # Payload = Total - IP header - Transport header
        payload_length = total_length - ip_header_size - transport_header_size

        return max(0, payload_length)  # Ensure non-negative
