...
class FlowSession(DefaultSession):
    ...
    def process(self, pkt: Packet):
        ...
        if flow is None:
            ...
            # Convert packet time to microseconds for comparison
            pkt_time_us = pkt.time * 1000000 if pkt.time < 1e10 else pkt.time
            while (pkt_time_us - flow.latest_timestamp) > expired:
                count += 1
                expired += EXPIRED_UPDATE
                with self._lock:
                    flow = self.flows.get((packet_flow_key, count))
    ...
    def garbage_collect(self, latest_time) -> None:
        ...
        for k in keys:
            ...
            # Convert latest_time to microseconds for comparison
            if latest_time is not None and latest_time < 1e10:
                latest_time_us = latest_time * 1000000
            else:
                latest_time_us = latest_time

            if not flow or (
                latest_time_us is not None
                and latest_time_us - flow.latest_timestamp
                < 5000000  # 5 seconds in microseconds
                and flow.duration < 120000000  # 2 minutes in microseconds
            ):
                continue
