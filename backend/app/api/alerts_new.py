...
async def bulk_update_alerts(request: BulkUpdateRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        count_check = db.execute(
            text(f"SELECT COUNT(*) FROM security_alerts WHERE id IN ({placeholders})"),
            id_params,
        ).scalar()
...
async def search_alerts(
    ...
    try:
        ...
        result = db.execute(text(search_query), {"search": f"%{q}%", "limit": limit})
...
async def bulk_delete_alerts(
    alert_ids: List[str] = Body(..., embed=True), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        result = db.execute(
            text(f"DELETE FROM security_alerts WHERE id IN ({placeholders})"), params
        )
        db.commit()
